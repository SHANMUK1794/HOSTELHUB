...
import KitchenExpensesAddForm from "./KitchenExpensesAddForm";
import KitchenExpensesUpdateForm from "./KitchenExpensesUpdateForm";
import KitchanExpensesExport from "./KitchanExpensesExport";
...
const KitchenExpenses = () => {
  ...
  const [filteredExpensesLocal, setFilteredExpensesLocal] = useState([]);
  ...
  const {
    data: kitchenExpenses,
    isLoading,
    addExpenseMutation,
    deleteExpenseMutation,
    updateExpenseMutation,
  } = useExpenses(apiYear, apiMonth, { showToast });
  ...
  useEffect(() => {
    let filtered = (kitchenExpenses ?? []).filter((expense) =>
    ...
    if (selectedYearMonth !== "all-all") {
      ...
      filtered = filtered.filter((expense) => {
        ...
        const [itemYear, itemMonth] = expense.date.split("-");
    ...
    setFilteredExpensesLocal(filtered);
  }, [kitchenExpenses, search, selectedYearMonth]);
  ...
  const totalPages = Math.ceil(filteredExpensesLocal.length / itemsPerPage);
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filteredExpensesLocal.slice(
    indexOfFirstItem,
    indexOfLastItem,
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="hidden md:block absolute right-0 bottom-0 top-0 w-1/4 lg:w-[30%] pointer-events-none">
          <div
      ...
      >
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center gap-5 mb-8">
          ...
          <div className="flex-1 w-full min-w-[250px] max-w-lg mx-auto">
            <SearchBar
              ...
              containerClass="flex items-center rounded-full px-4 py-2 border w-full shadow-sm transition-colors"
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto shrink-0 justify-center lg:justify-end">
            {(user?.role === "Admin" || user?.role === "Chef") && (
              ...
              >
                <FaPlus className="text-lg" /> Add Items
              </button>
            )}
            <div className="h-[40px]">
              <KitchanExpensesExport year={year} month={month} showToast={showToast} />
            </div>
          </div>
        </div>
              ...
              {isLoading ? (
                ...
                currentItems.map((expense, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center">{expense.unit}</td>
                    <td className="px-6 py-4 text-center">
                      {expense.quantity}
                    </td>
                    <td className="px-6 py-4 text-center">{expense.price}</td>
        ...
        {/* PAGINATION CONTROLS */}
        {!isLoading && filteredExpensesLocal.length > 0 && (
          <div className="flex flex-col sm:flex-row justify-between items-center mt-6 gap-4 px-2">
            ...
            >
              ...
              {Math.min(indexOfLastItem, filteredExpensesLocal.length)} of{" "}
              {filteredExpensesLocal.length} entries
            </span>
      ...
      {isOpenAdd && (
        <div className="fixed top-[87px] bottom-0 left-0 right-0 z-[9990] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 pt-0 pb-10">
          <KitchenExpensesAddForm setIsOpenAdd={setIsOpenAdd} showToast={showToast} />
        </div>
      )}
      {isOpenEdit && selectedExpense && (
        <div className="fixed top-[87px] bottom-0 left-0 right-0 z-[9990] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 pt-0 pb-10">
          <KitchenExpensesUpdateForm
            setIsOpenEdit={setIsOpenEdit}
            selectedEditData={selectedExpense}
            updateExpenseMutation={updateExpenseMutation}
          />
        </div>
      )}
      {isOpenDelete && selectedExpense && (
        <div className="fixed top-[87px] bottom-0 left-0 right-0 z-[9990] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 pt-0 pb-10">
          <Delete
            setIsOpenDelete={setIsOpenDelete}
            selectedId={selectedExpense._id}
            deleteData={handleDeleteExpense}
            refetch={() => deleteExpenseMutation.reset()}
          />
        </div>
      )}
...
export default KitchenExpenses;
