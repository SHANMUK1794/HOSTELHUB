...
import useStoreRoomExport from "../../hooks/useStoreRoomExport";
import { BsDownload } from "react-icons/bs";
import { XMarkIcon, CloudArrowDownIcon } from "@heroicons/react/24/solid";

function Export({ year, month, btnClass }) {
  const [showConfirm, setShowConfirm] = useState(false);
  const { store } = useStoreRoomExport();
  ...
  return (
    <>
      <button
        ...
        className={
          btnClass ||
          "flex items-center justify-center text-white px-4 py-[5px] gap-2 rounded-lg hover:opacity-90 transition"
        }
