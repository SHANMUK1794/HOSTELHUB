...
import useAdvanceExport from "../../hooks/useAdvanceExport";
import { XMarkIcon, CloudArrowDownIcon } from "@heroicons/react/24/solid";

function AdvanceExport({ branchName, showToast }) {
  const [showConfirm, setShowConfirm] = useState(false);
  const { advanceExport } = useAdvanceExport();

  const handleDownload = () => {
    advanceExport(branchName, {
  ...
  return (
    <>
      <button
        ...
        className="text-white px-5 py-3 rounded-xl flex items-center gap-2 shadow-md font-medium text-[15px] transition-all duration-200 hover:opacity-90"
