...
import useKitchenInventoryExport from "../../hooks/useKitchenInventoryExport";
import { XMarkIcon, CloudArrowDownIcon } from "@heroicons/react/24/solid";

function ExportInventory({ year, month, showToast }) {
  const [showConfirm, setShowConfirm] = useState(false);
  const { inventory } = useKitchenInventoryExport();
  ...
  const handleDownload = () => {
    ...
    inventory({ year, month, showToast });
  ...
  return (
    <>
      <button
        ...
        className="text-white px-5 py-2.5 rounded-lg text-sm font-medium flex items-center justify-center gap-2 shadow-sm transition-colors hover:opacity-90"
        style={{ background: "var(--theme-button-bg)" }}
      >
        Download <BsDownload strokeWidth={0.5} className="text-lg" />
      </button>
